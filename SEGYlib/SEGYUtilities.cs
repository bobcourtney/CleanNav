...
namespace SEGYlib
{
    ...
    public class SEGYUtilities
    {
        ...
        /// <summary>
        /// convert a SEGY trace header depth or election o to scalled length
        /// </summary>
        /// <param name="x">segy rev 1 trace header position</param>
        /// <param name="scalarToBeAppliedToAllCoordinates">segy rev 1 trace header    scalarForAllElevationsAndDepths </param>
        /// <returns>a decimal position calculated using scalarForAllElevationsAndDepths </returns>
        /// 
        /// 
        public static double convertToLength(int x, double scalarForAllElevationsAndDepths)
        {

            double d = (double)x;

            // apply scalar
            if (scalarForAllElevationsAndDepths < 0)
            {
                d /= -scalarForAllElevationsAndDepths;
            }
            else if (scalarForAllElevationsAndDepths > 0)
            {
                d *= scalarForAllElevationsAndDepths;
            }
